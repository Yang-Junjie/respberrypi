...
def convolution(image, kernel):
    ...
    # 在输入图像上滑动卷积核，并将每次卷积的结果保存在输出图像的相应位置上
...
def max_pooling(image, pool_size=(3, 3)):
    ...
    for i in range(output_height):
        for j in range(output_width):
            patch = [image[i*pool_height+k][j*pool_width:j*pool_width+pool_width] for k in range(pool_height)]
            output[i][j] = max([max(patch[k]) for k in range(pool_height)])
...
if __name__=='__main__':
    ...
    while(1):
        ...
        #文本信息
        LCD.text(f'This number',Y,Y-35,1)
        ...
        if(keyB.value() == 0):
            ...
            data = convolution(data, kernel)
            data = max_pooling(data, pool_size=(2, 2))
            
#             with open(f'data/trian_data/9_{count}.json', 'w') as f:
#                   json.dump(data, f)
            ...
            result_list = data_sort(no_deal_data,4)
